...
def load_corpus(pattern="corpus.txt"):
    ...
    corpus_pattern = Path(pattern)
    if not corpus_pattern.is_absolute():
        corpus_pattern = script_dir / corpus_pattern

    texts = []
    for path in glob.glob(str(corpus_pattern)):
...
class LM(nn.Module):
    ...
    def forward(self, x):
        ...
        e = self.drop(self.embed(x) + self.pos_embed(positions))
